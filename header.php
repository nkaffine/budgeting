 ...
 */
    ...
    function getHeaderInfo($page1, $page2){
        ...
        while($row = @ mysqli_fetch_array($result)){
            ...
            $display = commaSeparate($balance);
            if($row['balance_type'] == 1 && $balance > 0){
                $display = "(".$display.")";
            }
            $link = "account.php?account_id=" . $row['account_id'];
            $menu = $menu . "<li><a href='" . $link . "'><div style='font-weight: bold;'>" . $name . "</div><div>$" .$display. "</div></a></li>";
        }
        $menu = $menu . "</ul></li>";
        $menu = array($menu);
                ...
                <div class='container-fluid'>
                    ...
                    <div class='collapse navbar-collapse' id='myNavbar'>
                        ...
                        <ul class='nav navbar-nav navbar-right'>
                            <li class='dropdown'><a role='button' class='dropdown-toggle' data-toggle='dropdown'>Equity: {$menu[1]}<span class='caret'></span></a>
                                <ul class='dropdown-menu'>
